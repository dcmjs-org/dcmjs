...
```javascript
// Uncompressed pixel data is delivered as typed arrays
// Single frame: one buffer
// Multi-frame: multiple buffers via listener callbacks
...
// Each frame is delivered as an ArrayBuffer or `ArrayBuffer[]`
// Fragmented single frame or video arrays will be delivered as `ArrayBuffer[]` containing at least one array buffer per fragment.
// Fragmented multiframe requires the offset table to be present in order to be detected as single frame.
// Fragmented or large video will be delivered as an array of array buffers.
// Non-fragmented frames MAY be delivered as an array of arrays.
...
async function readCompressedImage(arrayBuffer) {
  ...
  // listener.frames contains each compressed frame
...
tag starting at 0.  The value of both uncompressed and compressed image frames
is one of:

   - BulkDataURI
   - ArrayBuffer containing the frame data
   - ArrayBuffer[] containing the frame data, or for video, all the frames

The array buffer array can be delivered EITHER as `ArrayBuffer[]` or as 
a sequence of calls:

```
  listener.startArray();
  frame.forEach(fragment => listener.value(fragment));
  listener.pop();
```
