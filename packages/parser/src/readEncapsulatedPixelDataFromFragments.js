...
export default function readEncapsulatedPixelDataFromFragments (dataSet, pixelDataElement, startFragmentIndex, numFragments, fragments) {
  ...
  const isBuffer = (typeof Buffer !== 'undefined') && (byteStream.byteArray instanceof Buffer);

  for (let i = startFragmentIndex; i < startFragmentIndex + numFragments; i++) {
    const fragmentOffset = fragmentZeroPosition + fragments[i].offset + fragmentHeaderSize;
    const fragmentLength = fragments[i].length;

    if (isBuffer) {
      byteStream.byteArray.copy(pixelData, pixelDataIndex, fragmentOffset, fragmentOffset + fragmentLength);
    } else {
      pixelData.set(byteStream.byteArray.subarray(fragmentOffset, fragmentOffset + fragmentLength), pixelDataIndex);
    }

    pixelDataIndex += fragmentLength;
  }
