...
export default function findItemDelimitationItemAndSetElementLength (byteStream, element) {
  ...
  // No item delimitation item - record a warning, set the length to the end of the buffer and set the position past the end of the buffer
  byteStream.warnings.push(`eof encountered before finding item delimiter while reading element of undefined length with tag ${element.tag}`);
