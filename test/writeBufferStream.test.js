...
describe("WriteBufferStream Tests", () => {
    ...
    it("writeBigInt64", () => {
        ...
        const expected = [];
        for (let i = 0; i < 512; i++) {
            expected[i] = BigInt(i - 256) * BigInt("0x1fffffffffffff");
            stream.writeBigInt64(expected[i]);
        }
        expect(stream.view.buffers.length).toBe(Math.ceil((512 * 8) / 25));
        for (let i = 0; i < 512; i++) {
            const actual = stream.view.getBigInt64(
                i * 8,
                stream.isLittleEndian
            );
            expect(actual).toBe(expected[i]);
    ...
    describe("readWorksAfterWrite", () => {
        ...
        out.writeBigInt64(BigInt(-123456789));
        out.writeUint32(123);
        out.writeUint16(234);
        out.writeUint8(25);
        ...
        const checkValues = stream => {
            ...
            expect(stream.readBigInt64()).toBe(BigInt(-123456789));
