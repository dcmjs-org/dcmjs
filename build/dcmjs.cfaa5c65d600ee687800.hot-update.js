...
var Segmentation = function () {
  ...
  _createClass(Segmentation, null, [{
    ...
    value: function generateToolState(imageIds, images, brushData) {
      ...
      var bitArray = BitArray.pack(cToolsPixelData);

      // Fill up the PixelData array with bitpacked segment information.
      for (var i = 0; i < masks.length; i++) {
        if (masks[i]) {
          var _bitArray = dcmjs.data.BitArray.pack(masks[i]);

          for (var j = 0; j < lengthOfCubeInBytes; j++) {
            pixels[i * lengthOfCubeInBytes + j] = _bitArray[j];
          }
        }
      }
...
//# sourceMappingURL=dcmjs.cfaa5c65d600ee687800.hot-update.js.map